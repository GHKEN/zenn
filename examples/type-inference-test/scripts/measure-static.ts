...
// 事前生成型定義のパターンでビルド時間を計測
const measureStatic = async () => {
	console.log("📊 Measuring build time with pre-generated types...");
	...
	for (let i = 1; i <= 3; i++) {
		...
		try {
			await execAsync(
				"tsc --noEmit --incremental false --project tsconfig.measure-static.json",
			);
	...
	console.log(`\n📈 Pre-generated types: ${average.toFixed(2)}s`);

...
Honoは、エンドポイント定義からAPIクライアントの型を自動生成できる機能を持っています。
...
- **zodのスキーマが数百個** → バリデーションルールの型推論が重い
- **Drizzleのテーブル定義が数十個** → リレーションを含む型推論が複雑
- **これらの組み合わせ** → 型の深さが指数関数的に増加

結果として：

- エディタの補完に数秒かかる（ひどい時は10秒以上）
- TypeScriptサーバーが頻繁にクラッシュ
- ビルド時間も増大（45秒 → 2分以上）
...
| tsgo（Rust製TypeScriptコンパイラ） | 0.92秒 | **約2.6倍高速** |
...
[@typescript/native-preview](https://github.com/microsoft/TypeScript/tree/native-preview)（通称tsgo）は、Rust製のTypeScriptコンパイラです。測定結果では約2.6倍の高速化が確認できました。
